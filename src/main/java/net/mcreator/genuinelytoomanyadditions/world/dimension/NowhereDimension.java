...
import net.mcreator.genuinelytoomanyadditions.procedures.NowherePlayerEntersDimensionProcedure;
import net.mcreator.genuinelytoomanyadditions.item.NowhereItem;
import net.mcreator.genuinelytoomanyadditions.block.NowhereStoneBlock;
import net.mcreator.genuinelytoomanyadditions.block.NowhereFramingsBlock;
...
public class NowhereDimension extends SoeModElements.ModElement {
	@ObjectHolder("soe:nowhere_portal")
	public static final CustomPortalBlock portal = null;
	public NowhereDimension(SoeModElements instance) {
		super(instance, 43);
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				ObfuscationReflectionHelper.setPrivateValue(WorldCarver.class, WorldCarver.CAVE, new ImmutableSet.Builder<Block>()
						...
						.add(NowhereStoneBlock.block.getDefaultState().getBlock()).build(), "field_222718_j");
	...
	public void clientLoad(FMLClientSetupEvent event) {
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				...
				effectsRegistry.put(new ResourceLocation("soe:nowhere"), customEffect);
			} catch (Exception e) {
				e.printStackTrace();
			}
		});
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("nowhere_portal", Vector3i::compareTo, 300);
	public static class POIRegisterHandler {
		...
		public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
			poi = new PointOfInterestType("nowhere_portal", Sets.newHashSet(ImmutableSet.copyOf(portal.getStateContainer().getValidStates())), 0, 1)
					.setRegistryName("nowhere_portal");
	...
	public void initElements() {
		...
		elements.items.add(() -> new NowhereItem().setRegistryName("nowhere"));
	}
	public static class CustomPortalBlock extends NetherPortalBlock {
			...
			if (random.nextInt(110) == 0)
				world.playSound(pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5,
						(net.minecraft.util.SoundEvent) ForgeRegistries.SOUND_EVENTS
								.getValue(new ResourceLocation(("entity.skeleton_horse.ambient_water"))),
		...
		public void onEntityCollision(BlockState state, World world, BlockPos pos, Entity entity) {
			if (!entity.isPassenger() && !entity.isBeingRidden() && entity.isNonBoss() && !entity.world.isRemote && true) {
				...
				} else if (entity.world.getDimensionKey() != RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("soe:nowhere"))) {
					entity.func_242279_ag();
					teleportToDimension(entity, pos, RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("soe:nowhere")));
				} else {
					entity.func_242279_ag();
	...
	public static class CustomPortalSize {
		private static final AbstractBlock.IPositionPredicate POSITION_PREDICATE = (state, blockReader, pos) -> {
			return state.getBlock() == NowhereFramingsBlock.block.getDefaultState().getBlock();
		};
					...
					for (int k2 = 0; k2 < 2; ++k2) {
						for (int i3 = -1; i3 < 3; ++i3) {
							BlockState blockstate1 = i3 < 0
									? NowhereFramingsBlock.block.getDefaultState().getBlock().getDefaultState()
									: Blocks.AIR.getDefaultState();
			...
			for (int k1 = -1; k1 < 3; ++k1) {
				for (int i2 = -1; i2 < 4; ++i2) {
					if (k1 == -1 || k1 == 2 || i2 == -1 || i2 == 3) {
						...
						this.world.setBlockState(blockpos$mutable, NowhereFramingsBlock.block.getDefaultState().getBlock().getDefaultState(), 3);
					}
				}
			}
	...
	public void onPlayerChangedDimensionEvent(PlayerEvent.PlayerChangedDimensionEvent event) {
		...
		if (event.getTo() == RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("soe:nowhere"))) {
			{
				Map<String, Object> $_dependencies = new HashMap<>();
				$_dependencies.put("entity", entity);
				NowherePlayerEntersDimensionProcedure.executeProcedure($_dependencies);
			}
		}
	}
}
